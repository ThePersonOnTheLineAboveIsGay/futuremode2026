...
Extension popup 有一個「整理重點（Debug）」按鈕，會請後端把**從會議開始到現在**的完整逐字稿（不是插話判斷用的那個 15 分鐘滾動視窗）整理成一份按時間先後排列的重點摘要。結果只會回給按按鈕的那個人，由**他自己的連線**嘗試貼進 Meet 聊天室（不透過插話用的那個「指定聊天室發送端」機制，這樣不管是不是那個發送端，按下去都保證會嘗試發聊天室）；貼失敗才會改成跳浮動卡片。如果還沒有逐字稿、或整理失敗，只有按按鈕的人自己會看到診斷訊息，不會發進聊天室。用來確認「AI 到底聽到了什麼」，不影響插話判斷邏輯。
...
還沒有逐字稿、或整理失敗時，一樣只回給發出請求的那條連線，但不帶 `send_to_chat`（Extension 收到後直接跳浮動卡片，不會嘗試發聊天室，避免診斷訊息或錯誤訊息洗版）。
