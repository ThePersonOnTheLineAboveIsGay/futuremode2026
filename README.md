...
在 Google Meet 會議進行中，一個 AI 持續聆聽對話；當它判斷某個提案「不可行」或「有疑慮」時，
即時在 Meet 頁面左下角彈出理由卡片。
...
Google Meet 分頁
  └─ Chrome 擴充功能 (MV3)
       ...
       └─ content script：左下角浮層顯示「不可行／有疑慮提案 + 理由」
                    │
FastAPI 後端 ───────┘
  ├─ OpenRouter（Whisper，音訊 → 文字）   逐段轉錄 → 滾動逐字稿
  └─ Gemini API（結構化 JSON）           每 ~30s 分析最近視窗
       verdict=infeasible（confidence≥門檻）或 needs_info，且未回報過 → 推播
```
...
4. 進入一場 Google Meet，左下角會出現深色面板（含除錯記錄）。
5. **點 Chrome 工具列上的綠色方塊圖示** → 開始監聽（首次會要求選擇並允許擷取此分頁）。
   - 監聽中時圖示會有綠色 ● 標記。
   - 偵測到不可行（紅框）或有疑慮（黃框）的提案時面板會展開並新增理由卡片，**同時預設會自動把這則提醒發到 Meet 聊天室**（所有與會者都看得到；可在「選項」頁關掉）。
6. 再點一次圖示 → 停止。
...
4. 講出一個明顯不可行（或有疑慮）的提案 → 約 30–60 秒內左下角浮層出現理由卡片。
