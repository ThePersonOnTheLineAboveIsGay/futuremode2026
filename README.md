...
**Meet 會議小甜心是 Google Meet 裡的即時會議夥伴：它聽懂討論，在有人前後說法矛盾、明顯離題、有可核對的邏輯問題，或提出方案／意見值得表態時，主動給出它的判斷與建議。**
...
| 會議浮動卡 | 右上深色卡片、黃色標籤「提醒／偏題／重點／建議」、信心百分比、對象、短提醒、條列理由 | AI 給可行動且可核對的提示 |
...
| 20–33 | 浮動卡平順出現：標籤「提醒」、對象「小美」、訊息「稍早提到採用方案 A，現在改為方案 B；要說明改變原因嗎？」 | 「Meet 會議小甜心會理解近期脈絡，只在有明確證據時，溫和提醒。」 |
...
| 80–90 | 回到完整 Meet 畫面與產品名稱。 | 「Meet 會議小甜心：讓每個決策，都經得起下一句話。」 |
...
```json
{
  ...
  "quote": "那我們就先用方案 B 好了",
  "message": "🤖 AI 提醒：稍早提到要用方案 A，現在說的是方案 B，要說明改變原因嗎？",
  "confidence": 0.82,
  "send_to_chat": true
}
```

`target_speaker` 只有在該發言有對應真實顯示名稱時才會指名；完全沒有身分資訊時、以及分頁混音經聲紋分辨後配到的匿名講者，一律是 `null`（後者只用來讓內部前後矛盾比對能分得清是誰、誰，不會外洩到任何欄位或訊息內容）。`message` 本文（也是送進 Meet 聊天室的那段文字）刻意不會提到任何人名——是誰的資訊只在浮動卡片獨立的「對象：」欄位顯示，不管有沒有身分都一樣。`reasons` 是條列的具體理由（最多 3 點），`quote` 是最新發言逐字稿中觸發此判斷的原文片段。`send_to_chat` 只會對同房被選為聊天室發送端的那條連線設為 `true`。
...
**選用：匿名講者分辨**（見「分頁混音的匿名講者分辨」一節）。不裝也完全不影響其他功能，只是分頁混音會維持全部匿名、不分你我：

```powershell
pip install --no-deps resemblyzer
pip install -r backend/requirements-diarization.txt
# 另外需要系統裝好 ffmpeg 並加進 PATH（解碼 webm/opus 用），例如 winget install ffmpeg
```
...
5. 裝置 A 應顯示插話浮動卡片；因為是靠分頁混音聽到的匿名發言，卡片跟訊息都不會指名任何人（真名或任何形式的代稱都不會出現），對象欄位維持「整體會議提醒」。裝了選用的聲紋分辨依賴時，後端內部還是能分清楚「這是同一個人前後矛盾」跟「這是不同人各自的意見」，只是不會把分辨結果顯示出來。
...
分頁混音（沒裝 Extension 的人、或裝了但沒填顯示名稱的麥克風）本來會把所有聲音混成同一個「匿名來源」，如果現場有 3 個人甚至更多人共用同一個來源講話，AI 完全看不出是誰講的，容易把「不同人講不同意見」誤判成「會議內容自己前後矛盾」。

裝了選用依賴（見上面的本機開發設定）後，後端會對每一段已經切好的匿名逐字稿抽一次聲紋（`backend/app/diarization.py`），跟這個房間裡先前出現過的聲紋比對相似度：像就歸進同一個內部分組標籤，不像就開一個新的標籤。這個標籤純粹是後端內部用來分組的識別碼，**不會**出現在浮動卡片、聊天室訊息或任何 AI 回覆內容裡——AI 本身也看不到這個標籤文字，只看得到「這幾句話是同一位不具名的人講的」這個分組結果。效果是同一個人前後矛盾還是能被追蹤、抓到，同時不會把不同人的意見誤判成矛盾，但畫面上看起來跟完全匿名時一樣（都不指名）。這是「輪流發言」的分辨，**不是**完整的語者分離：同一段音訊裡如果有人搶話、同時講，沒辦法把重疊的聲音拆開；沒裝選用依賴、或系統缺 ffmpeg 時，會自動退回原本「全部匿名」的行為，不會出錯或中斷辨識。
...
- 只有裝了 Extension 的人才有乾淨的個人音軌與講者身分，也只有他們的插話提醒會指名。沒裝的人靠分頁混音備援被匿名聽到，一律不指名；裝了選用的聲紋分辨依賴時，後端內部能分清楚是不是同一人前後矛盾（不會誤判成不同人的意見互相矛盾），但這個分辨結果不會顯示出來。沒裝依賴、或連分頁混音都拿不到（例如 tabCapture 權限失敗）時，就完全依賴聊天室訊息。
