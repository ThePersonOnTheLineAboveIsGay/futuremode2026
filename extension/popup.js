...
async function init() {
  const saved = await chrome.storage.local.get([
    "listening", "websocketUrl", "ttsEnabled",
...
document.querySelector("#stop-voice").addEventListener("click", async () => {
  errorEl.textContent = "";
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  if (!tab?.url?.startsWith("https://meet.google.com/")) {
    errorEl.textContent = "請先切到 Google Meet 分頁。";
  ...
  try {
    await chrome.tabs.sendMessage(tab.id, { target: "content", type: "stop-voice" });
  } catch (error) {
