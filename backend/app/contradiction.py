...
4. 提出了具體方案、計畫或決定——不論是「在多個選項之間選擇」，還是只提出「單一做法」（例如宣稱要在某個時間內完成某件事、採用某種做法）。此時根據逐字稿內容與常識，列出優缺點、風險，特別留意時程、資源或技術上明顯不合理的地方（例如聲稱能在極短時間內完成通常需要長得多時間的工作、明顯超出預算或能力範圍的計畫）。只呈現論點與風險，絕對不要說「應該怎麼做」或替團隊下結論，是否採納由團隊自己判斷。
...
第 4 類（方案討論）不需要等到有兩個選項互相比較才觸發，單一個方案或計畫只要有明顯可質疑之處（例如時程、成本、技術可行性）就可以觸發；列出的優缺點與風險必須是逐字稿裡實際提到，或根據常識可以直接合理推論的內容，不可以編造沒提過的具體細節，也不可以下「哪個比較好」或「應該這樣做」的結論。
